...
import { shouldUseJamunAIAPI } from "./jamunai.api";

// JamunAI STT function
async function fetchJamunAISTT(audio: File | Blob): Promise<string> {
  try {
    ...
    return `JamunAI STT Error: ${errorMessage}`;
  }
}
...
export async function fetchSTT(params: STTParams): Promise<string> {
  ...
  try {
    ...
    // Check if we should use JamunAI API instead
    const useJamunAIAPI = await shouldUseJamunAIAPI();
    if (useJamunAIAPI) {
      return await fetchJamunAISTT(audio);
    }
