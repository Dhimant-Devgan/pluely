...
export const useCompletion = () => {
  ...
  const submit = useCallback(
    async (speechText?: string) => {
      ...
      try {
        ...
        const useJamunAIAPI = await shouldUseJamunAIAPI();
        // Check if AI provider is configured
        if (!selectedAIProvider.provider && !useJamunAIAPI) {
          setState((prev) => ({
            ...prev,
        ...
        try {
          ...
          for await (const chunk of fetchAIResponse({
            provider: useJamunAIAPI ? undefined : provider,
  ...
  useEffect(() => {
    ...
    const handleStorageChange = async (e: StorageEvent) => {
      if (e.key === "jamunai-conversation-selected" && e.newValue) {
        try {
        ...
        if (!hasPermission) {
          ...
          if (!hasPermissionNow) {
            setState((prev) => ({
              ...prev,
              error:
                "Screen Recording permission required. Please enable it by going to System Settings > Privacy & Security > Screen & System Audio Recording. If you don't see JamunAI in the list, click the '+' button to add it. If it's already listed, make sure it's enabled. Then restart the app.",
            }));
